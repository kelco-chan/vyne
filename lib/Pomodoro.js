...
class Pomodoro {
    ...
    update() {
        ...
        this.lastUpdateTime = Date.now();
        prisma_1.default.session.findUnique({ where: { id: this.id } }).then(async (session) => {
            let newMembers = (await this.getCurrentMembers()).map(member => member.id);
            let newMemberList = [...new Set([...newMembers, ...(session?.participantIds || [])])];
            prisma_1.default.session.update({
                where: { id: this.id },
                data: {
                    timeCompleted: this.timeCompleted,
                    participantIds: newMemberList
                }
            });
        });
    }
    ...
    getStatusPayload(seekAhead) {
        ...
        return {
            ...
            components: (status.type === "WORK") ? [] : [new discord_js_1.MessageActionRow().addComponents(new discord_js_1.MessageButton()
